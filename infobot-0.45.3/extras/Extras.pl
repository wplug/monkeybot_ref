...
&status("Karma string is currently \'$message2\'");
  ...
  #################################################################
  # Guessing Game donald.g.park@iname.com
  #################################################################
    
    if ($message =~ /^guess\s+(\w+)/) {
	my $reply, $number, $ingame, $startguy, %count;
	my $command = $1;
	if ($command eq "begin" && $ingame == 0) {
	    $reply = "Im thinking of a number between 1 and 100";
	    $number = int(rand 100) + 1;
	    $ingame = 1;
	    $startguy = $who;
	    %count = ();
	} elsif ($command eq "begin" && $ingame == 1) {
	    $reply = "Im already playing a game started by $startguy";
	} elsif ($command > $number && $ingame == 1) {
	    $reply = "$who: Too high";
	    $count[$who]++;
	} elsif ($command > $number && $ingame == 0) {
	    $reply = "$who: I havent started a game. say guess begin to start.";
	} elsif ($command < $number && $ingame == 1) {
	    $reply = "$who: Too low";
	    $count[$who]++;
	} elsif ($command > $number && $ingame == 0) {
	    ...
	    $reply = "$who: $number is what i was thinking of!\n$who: You took $count[$who] guesses.";
	    $count[$who] = 0;
	    $ingame = 0;
	} elsif ($command == $number && $ingame == 0) {
	    $reply = "$who: $number is it, but you're a little late. say guess begin to start over.";
	}
	return $reply;
    }
    return undef;
