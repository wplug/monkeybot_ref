...
sub irc {
	...
	while (1) {
		...
		if (sysread(SOCK,$buf,1) <= 0) {
		    # So, for some reason, when the bot recieves a SIGCHLD,
		    # it reads zero bytes from the socket.  This made the 
		    # while loop restart, and initiate a new tcp socket.
		    # This is broken, so I just have it try 4 times before
		    # restarting.  Stupid hack.  Why 4 times?  I meant to
		    # type 1 but hit 3 instead.
		    # billings 7-10-2006
		    &status("Read zero from socket.");
		    $readnull++;
		    last if ($readnull > 3);
		}
		if ($buf=~/\n/) {
